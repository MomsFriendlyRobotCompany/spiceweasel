...
public:
  cVector(int, const char * = NULL, ml_data * = NULL);
  cVector(void) {
    size = 0;
    name = NULL;
    p    = NULL;
  ...
  inline bool operator==(const cVector &a) {
    return (size == a.size ? true : false);
  }
  inline bool operator!=(const cVector &a) {
    return (size == a.size ? false : true);
  }
  ...
  inline void set(ml_data a, ml_data b, ml_data c) {
    p[0] = a;
    p[1] = b;
    p[2] = c;
  }
...
class cMatrix : public cBaseMath {
  friend cMatrix &operator+(const cMatrix &, const cMatrix &); // matrix
                                                               // addition
  friend cMatrix &operator-(const cMatrix &,
                            const cMatrix &); // matrix subtraction
  friend cMatrix &operator-(const cMatrix &); // negate a matrix
  friend cMatrix &operator*(const cMatrix &,
                            const cMatrix &);          // matrix multiplication
  ...
  friend cVector &operator*(const cMatrix &, const cVector &); // matrix
                                                               // addition
  ...
  friend std::ostream &operator<<(std::ostream &s,
                                  const cMatrix &mat); // print matrix

public:
  cMatrix(int, int, const char * = NULL, const ml_data * = NULL); // constructor
  cMatrix(void) {
    r = c = 0;
    name  = NULL;
    p     = NULL;
  ...
  inline bool operator==(const cMatrix &a) {
    return (r == a.r && c == a.c ? true : false);
  }
  inline bool operator!=(const cMatrix &a) {
    return (r == a.r && c == a.c ? false : true);
  }
  ...
  inline void set(const cMatrix &m, int a, int b) {
    set(m.p, a, b, m.r, m.c);
    m.releaseTmp();
  }
...
cMatrix &skew(const ml_data *, int = 3);
cMatrix &eye(int);
cMatrix &zeros(int, int);
