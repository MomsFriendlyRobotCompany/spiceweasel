 ...
 */
cVector::cVector(int s, const char *new_name, ml_data *a) {
  type = ML_VECTOR;
  key  = 0;
  size = s;
  name = NULL;
...
#ifdef ML_CHECK
  if (p == NULL) {
    // sprintf(errMsg,"%s could not allocate memory for data
...
#ifdef ML_CHECK
  if (v.size != size) {
    // sprintf(errMsg,"%s vector sizes do not match
...
cVector &operator+(const cVector &left, const cVector &right) {
  ...
  if (left.size != right.size) {
    // sprintf(errMsg,"%s can't add vectors %s +
    // %s",__PRETTY_FUNCTION__,left.name,right.name);
    throw cMLError(cMLError::FATAL, errMsg);
  }
#endif
...
#ifdef ML_CHECK
  if (left.size != right.size) {
    // sprintf(errMsg,"%s can't subtract vectors %s -
    // %s",__PRETTY_FUNCTION__,left.name,right.name);
    throw cMLError(cMLError::FATAL, errMsg);
  }
#endif
...
#ifdef ML_CHECK
  if (a.size != b.size) {
    // sprintf(errMsg,"%s MUST be the same size, out =
    // dot(%s(%d),%s(%d))",__PRETTY_FUNCTION__,a.name,a.size,b.name,b.size);
    throw cMLError(cMLError::FATAL, errMsg);
  }
#endif
...
cVector &cross(const cVector &a, const cVector &b) {
  ...
  if (a.size != 3 || b.size != 3) {
    // sprintf(errMsg,"%s vectors MUST be of size 3, out =
    // dot(%s(%d),%s(%d))",__PRETTY_FUNCTION__,a.name,a.size,b.name,b.size);
    throw cMLError(cMLError::FATAL, errMsg);
  }
#endif
