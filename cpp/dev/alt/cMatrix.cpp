...
/////////////////////////////////////////////////////////////////
cMatrix::cMatrix(int rows, int cols, const char *n, const ml_data *d) {
  r         = rows;
  c         = cols;
  int range = r * c;
  type      = ML_MATRIX;
...
#ifdef ML_CHECK
  if (p == NULL) {
    snprintf(errMsg, 255, "%s could not allocate memory for data %s\n",
...
void cMatrix::resize(int a, int b) {
  ...
  if (p == NULL) {
    snprintf(errMsg, 255,
             "%s couldn't create enough memory when resizing matrix %s",
             __PRETTY_FUNCTION__, name);
    throw cMLError(cMLError::FATAL, errMsg);
  }
#endif
...
cMatrix &cMatrix::eye(void) {
  ...
  if (r != c) {
    snprintf(errMsg, 255, "%s r != c", __PRETTY_FUNCTION__);
    throw cMLError(cMLError::FATAL, errMsg);
  }
...
cVector &operator*(const cMatrix &a, const cVector &b) {
  ...
  if (a.c != b.size) {
    snprintf(errMsg, 255,
             "%s can't multiply matrix and vector %s(%d,%d) * %s(%d)",
             __PRETTY_FUNCTION__, a.name, a.r, a.c, b.name, b.size);
    throw cMLError(cMLError::FATAL, errMsg);
  }
#endif
...
cMatrix &cMatrix::operator=(const cMatrix &mat) {
  ...
  if (mat.r != r || mat.c != c) {
    snprintf(errMsg, 255,
             "%s operator= matrix sizes do not match %s(%d,%d)=%s(%d,%d)\n",
...
cMatrix &operator+(const cMatrix &m1, const cMatrix &m2) {
  ...
  if (m1.r != m2.r || m1.c != m2.c) {
    snprintf(errMsg, 255, "%s matrix sizes do not match %s(%d,%d)+%s(%d,%d)\n",
...
cMatrix &operator-(const cMatrix &left, const cMatrix &right) {
  ...
  if (left.r != right.r || left.c != right.c) {
    snprintf(errMsg, 255, "%s unable to subtract matricies %s - %s\n",
...
#ifdef ML_CHECK
  if (a.c != b.r) {
    snprintf(errMsg, 255, "%s can't multiply matricies %s(%d,%d) * %s(%d,%d)\n",
...
cMatrix &cMatrix::inv(void) {
  ...
  if (r != c) {
    ...
    snprintf(errMsg, 255,
             "%s unable to invert matrix %s[%i x %i], must be square\n",
  ...
  for (i = 0; i < n; i++) { // main loop over the c to be reduced
    ...
    if (irow != icol) {
      for (l = 0; l < n; l++)
        std::swap(a->p[l + irow * a->c], a->p[l + icol * a->c]);
      for (l = 0; l < m; l++)
        std::swap(b->p[l + irow * b->c], b->p[l + icol * b->c]);
    }
    ...
    for (l = 0; l < n; l++)
      a->p[l + icol * a->c] *= pivinv;
    for (l = 0; l < n; l++)
      b->p[l + icol * b->c] *= pivinv;
    for (ll = 0; ll < n; ll++)
      if (ll != icol) {
        dum                    = a->p[icol + ll * a->c];
        a->p[icol + ll * a->c] = 0.0;
        for (l = 0; l < n; l++)
          a->p[l + ll * a->c] -= a->p[l + icol * a->c] * dum;
        for (l = 0; l < n; l++)
          b->p[l + ll * b->c] -= b->p[l + icol * b->c] * dum;
      }
  }
  for (l = n - 1; l >= 0; l--) {
    if (indxr[1] != indxc[l])
...
cMatrix &cMatrix::skew(const ml_data *b) {
...
#ifdef ML_CHECK
    if (r != 3 || c != 3) {
      snprintf(errMsg, 255, "%s %s(%d,%d) MUST be a semetric cMatrix",
               __PRETTY_FUNCTION__, name, r, c);
      throw cMLError(cMLError::FATAL, errMsg);
    }
#endif
  ...
  default:
#ifdef ML_CHECK
    snprintf(errMsg, 255, "%s %s.skew(%d,%d) can only do size 3 and 4\n",
             __PRETTY_FUNCTION__, name, r, c);
    throw cMLError(cMLError::FATAL, errMsg);
#endif
    break;
...
cMatrix &skew(const ml_data *b, int size) {
  ...
  case 4:
    ...
    break;
  default:
    snprintf(errMsg, 255, "%s skew(%d,%d) can only do size 3 and 4\n",
...
#ifdef ML_CHECK
  if (i * j <= 0 || i > r || j > c) {
    snprintf(errMsg, 255, "%s matrix(%d,%d) is out of bounds\n",
